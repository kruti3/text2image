...
def build_generator(input_noise, input_text, layer_list):
    ...
    ithconv_layer = Deconv2DLayer(ithconv_layer, 3, 5, stride=1, pad=2, nonlinearity=tanh)
    
    return ithconv_layer

def build_discriminator(input_img, input_text, layer_list):
    ...
    input_fc_dis = batch_norm(Conv2DLayer(input_fc_dis, layer_list[0], 5, stride=1, pad=2, nonlinearity=rectify))
...
def main(layer_list, num_epochs, loss_func):
    # Load the dataset
    initial_eta=1e-4
    print("Loading data...")
...
#    target_var = T.ivector('targets')
    ...
    generator = build_generator(noise_var, input_text, layer_list)
    discriminator = build_discriminator(input_img, input_text, layer_list)
    ...
    fake_out = lasagne.layers.get_output(discriminator,
            {all_layers[0]: lasagne.layers.get_output(generator), all_layers[2+3*len(layer_list-1)]: input_text})
...
if __name__ == '__main__':
    ...
    main(layer_list=args.layer_list, num_epochs=args.num_epochs, loss_func=args.loss_func)
